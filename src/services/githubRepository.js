...
async function request(path, signal) {
  ...
  if (!response.ok) {
    if (response.status === 403 || response.status === 429)
      throw new Error(
        "GitHub is temporarily rate-limiting requests. Try again later, or open the repository on GitHub.",
      );
    if (response.status === 404)
      throw new Error(
        "This public repository or file was not found on GitHub. It may have moved or become private.",
      );
    throw new Error(
      `GitHub could not load this resource (HTTP ${response.status}).`,
    );
  }
...
export async function getRepository(identity, signal) {
  if (!identity)
    throw new Error("No public GitHub repository was linked to this project.");
...
export async function getFile(identity, path, branch = "", signal) {
  ...
  if (result.type !== "file")
    throw new Error("This item is not a readable source file.");
  if (result.size > MAX_FILE_BYTES)
    throw new Error(
      "This file is too large to preview safely. Open it on GitHub instead.",
    );
  if (result.encoding !== "base64" || typeof result.content !== "string")
    throw new Error("GitHub did not return previewable text for this file.");
  const bytes = Uint8Array.from(
    atob(result.content.replace(/\s/g, "")),
    (char) => char.charCodeAt(0),
  );
  if (bytes.some((byte) => byte === 0))
    throw new Error("Binary files cannot be previewed as source code.");
  return new TextDecoder("utf-8", { fatal: false }).decode(bytes);
}

export function isPreviewableFile(entry) {
  if (!entry || entry.type !== "file" || entry.size > MAX_FILE_BYTES)
    return false;
  return (
    /^(?:README(?:\.[\w-]+)?|Dockerfile|Makefile|\.gitignore|\.env\.example)$/i.test(
      entry.name,
    ) ||
    /\.(?:js|jsx|ts|tsx|mjs|cjs|json|md|mdx|py|java|c|h|cpp|hpp|cs|go|rs|rb|php|html|htm|css|scss|sass|sql|yaml|yml|toml|sh|bash|txt|xml|svg|prisma|dart|vue|svelte|graphql|gql)$/i.test(
      entry.name,
    )
  );
}
...
export function pickInitialFile(entries) {
  return (
    entries.find(
      (entry) =>
        entry.type === "file" &&
        /^(?:app|index|main|server)\.[jt]sx?$/i.test(entry.name),
    ) ||
    entries.find(
      (entry) => isPreviewableFile(entry) && !/^readme/i.test(entry.name),
    ) ||
    entries.find(isPreviewableFile) ||
    null
  );
}
